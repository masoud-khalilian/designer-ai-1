...
def main():
    ...
    # init_prompt.display_current_prompt()

    remove_space = RemoveSpace() 
    summarize = Summarize()
    generation1 = PromptGeneration(name='prompt generation 1',overhead=cfg_p.gen1_overhead.value)
    module_list = [
        remove_space,
        # summarize,
        generation1
    ...
    ask_prelimniary_er = CallOpenAI(prompt=init_prompt.get_prompt(),system_prompt=cfg_p.api_call_system.value)
    ...
    generation2 = PromptGeneration(name='prompt generation 2',overhead=cfg_p.gen2_overhead.value)
    generation2(second_prompt)

    ask_json_array = CallOpenAI(prompt=second_prompt.get_prompt(),system_prompt=cfg_p.api_call_system2)
